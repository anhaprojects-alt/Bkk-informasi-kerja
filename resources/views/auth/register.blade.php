    ...
    <div class="max-w-md mx-auto min-h-screen bg-slate-50 flex flex-col justify-between p-6 shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-x border-slate-200/50">
        ...
        <div class="mt-2">
            ...
            <div class="mt-5 space-y-1">
                ...
                <p class="text-sm text-slate-500 font-medium">Gabung sekarang untuk mulai melamar pekerjaan.</p>
            </div>
        </div>

        <!-- Form Card (3D Floating Box) -->
        <form method="POST" action="{{ route('register.post') }}" class="my-auto space-y-4 bg-white p-5 rounded-3xl border border-slate-200/60 shadow-[0_10px_30px_rgba(0,0,0,0.04)]"
            x-dataClan="{ busy: false, showPassword: false }" x-data="{ busy: false, showPassword: false }" @submit="busy = true">
            @csrf

            <!-- Name -->
            <div class="space-y-1">
                <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-400">Nama Lengkap</label>
                <input type="text" name="name" id="name" required
                    value="{{ old('name') }}"
                    autocomplete="name" autocapitalize="words"
                    placeholder="Contoh: Budi Santoso"
                    @error('name') aria-invalid="true" aria-describedby="name-error" @enderror
                    class="block w-full px-4 py-3 bg-slate-50/80 border rounded-2xl text-sm focus:outline-none focus:bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:shadow-none transition duration-150 @error('name') border-red-300 focus:ring-2 focus:ring-red-100 @else border-slate-200/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 @enderror">
                @error('name')
                    <p id="name-error" class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-1">
                ...
                <input type="email" name="email" id="email" required
                    ...
                    class="block w-full px-4 py-3 bg-slate-50/80 border rounded-2xl text-sm focus:outline-none focus:bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:shadow-none transition duration-150 @error('email') border-red-300 focus:ring-2 focus:ring-red-100 @else border-slate-200/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 @enderror">
                @error('email')
            ...
            <!-- Phone Number -->
            <div class="space-y-1">
                <label for="phone_number" class="block text-xs font-bold uppercase tracking-wider text-slate-400">Nomor HP / WhatsApp</label>
                <input type="tel" name="phone_number" id="phone_number" required
                    value="{{ old('phone_number') }}"
                    autocomplete="tel" inputmode="tel"
                    placeholder="081234567890"
                    @error('phone_number') aria-invalid="true" aria-describedby="phone_number-error" @enderror
                    class="block w-full px-4 py-3 bg-slate-50/80 border rounded-2xl text-sm focus:outline-none focus:bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:shadow-none transition duration-150 @error('phone_number') border-red-300 focus:ring-2 focus:ring-red-100 @else border-slate-200/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 @enderror">
                @error('phone_number')
                    <p id="phone_number-error" class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>

            <!-- Role Select -->
            <div class="space-y-1">
                <label for="role" class="block text-xs font-bold uppercase tracking-wider text-slate-400">Daftar Sebagai</label>
                <div class="relative">
                    <select name="role" id="role" required
                        @error('role') aria-invalid="true" aria-describedby="role-error" @enderror
                        class="block w-full px-4 py-3 bg-slate-50/80 border rounded-2xl text-sm focus:outline-none focus:bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:shadow-none appearance-none transition duration-150 @error('role') border-red-300 focus:ring-2 focus:ring-red-100 @else border-slate-200/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 @enderror">
                ...
                @error('role')
                    <p id="role-error" class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    <input :type="showPassword ? 'text' : 'password'" type="password" name="password" id="password" required
                        ...
                        class="block w-full px-4 pr-12 py-3 bg-slate-50/80 border rounded-2xl text-sm focus:outline-none focus:bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:shadow-none transition duration-150 @error('password') border-red-300 focus:ring-2 focus:ring-red-100 @else border-slate-200/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 @enderror">
                ...
                <p id="password-hint" class="text-[11px] text-slate-400 font-medium">Minimal 8 karakter dengan kombinasi aman.</p>
                @error('password')
        ...
        <div class="mb-2 text-center">
            <p class="text-sm text-slate-500 font-medium">Sudah punya akun? <a href="{{ route('login') }}" class="font-bold text-blue-600 hover:text-blue-700 transition">Masuk di Sini</a></p>
        </div>
