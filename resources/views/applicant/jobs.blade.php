    ...
    <div class="max-w-md mx-auto min-h-screen bg-white flex flex-col shadow-xl">
        ...
        <header class="p-6 pb-4 sticky top-0 bg-white/90 backdrop-blur z-10 border-b border-slate-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-slate-400 font-semibold uppercase tracking-wider">Halo, {{ Auth::user()->name }}</p>
                    <h1 class="text-xl font-extrabold text-slate-900 mt-0.5">Temukan Lowongan</h1>
                </div>
                <span class="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-sm">{{ $jobs->count() }}</span>
            </div>

            <form method="GET" action="{{ route('jobs.index') }}" class="mt-4">
                <div class="relative">
                    <input type="text" name="q" value="{{ $search }}" placeholder="Cari posisi, lokasi, atau perusahaan..."
                        class="block w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:bg-white transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400 absolute left-3 top-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <main class="flex-1 overflow-y-auto p-6 pt-4 space-y-3.5 pb-24">
            ...
            @forelse ($jobs as $job)
                <a href="{{ route('jobs.show', $job) }}" class="block p-4 bg-white border border-slate-200 rounded-2xl shadow-sm hover:border-indigo-300 hover:shadow-md transition">
            ...
            @empty
                <div class="text-center py-16">
                    ...
                    <p class="text-sm text-slate-500 mt-4">Belum ada lowongan yang cocok.</p>
                </div>
            @endforelse
        </main>
