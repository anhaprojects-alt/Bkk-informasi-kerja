...
class DashboardController extends Controller
{
    ...
    private function scopedJobs()
    {
        ...
        return $user->role === 'admin'
            ? JobListing::query()
            : JobListing::where('company_id', optional($user->company)->id);
    }
    ...
    public function index()
    {
        ...
        $jobs = $this->scopedJobs()->withCount('applicants')->latest()->take(10)->get();
    ...
    public function createJob()
    {
        ...
        $companies = Auth::user()->role === 'admin' ? Company::orderBy('name')->get() : collect();
    ...
    public function storeJob(Request $request)
    {
        ...
        return redirect()->route('dashboard')->with('status', 'Lowongan kerja berhasil ditambahkan.');
    }

    public function closeJob(JobListing $jobListing)
    {
        $this->ensureStaff();
        $this->authorize('close', $jobListing);
        $jobListing->update(['status' => 'closed']);
        return redirect()->route('dashboard')->with('status', 'Lowongan kerja telah ditutup.');
    }
}
